...
@Component({
    ...
    <section class="contact-section">
      <div class="container">
        <div class="contact-cards">
          <a class="info-card email-card" href="mailto:egyptianspiritbooking@gmail.com">
            <i class="fa-solid fa-envelope"></i>
            <span>{{ 'CONTACT_PAGE.EMAIL_LABEL' | translate }}</span>
            <strong>egyptianspiritbooking@gmail.com</strong>
          </a>

          <a class="info-card" href="tel:+201106131756">
  ...
  styles: [`
    ...
    .info-card:hover {
      ...
      border-color: rgba(198, 168, 92, 0.55);
    ...
    :host-context([dir='rtl']) .mini-list div {
...
export class ContactComponent {
  ...
  private async sendContactEmail() {
    const templateParams = {
      ...
      trip_name: this.form.subject || 'Contact Message',
      travel_date: '-',
      guests: '-',
  ...
  private async sendContactAutoReply() {
    const templateParams = {
      request_type: 'Contact Message',
      user_name: this.form.name,
      user_email: this.form.email,
  ...
  async submit() {
    ...
    if (!name || !email || !message) {
      this.loading = false;
      this.errorMessage = 'Please fill in name, email, and message.';
      this.cdr.detectChanges();
      return;
    }

    const finalMessage = this.form.subject
      ? `${this.form.subject}\n\n${this.form.message}`
      : this.form.message;
    ...
    try {
      ...
      await this.sendContactAutoReply();
    } catch (emailError) {
      console.error('EmailJS error:', emailError);

      this.loading = false;
      this.errorMessage =
        'Message saved, but one of the email notifications was not sent.';
      this.cdr.detectChanges();
      return;
    }
